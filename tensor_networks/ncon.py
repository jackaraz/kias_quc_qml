""" ncon network contractor, taken from https://www.tensors.net/code"""
import numpy as np
...
def ncon(
    ...
    for ele in range(len(tensor_list)):
        ...
        if num_cont > 0:
            ...
            con_order = np.delete(
                con_order, np.intersect1d(con_order, cont_ind, return_indices=True)[1]
            )
    ...
    while len(con_order) > 0:
        ...
        con_order = np.delete(
            con_order, np.intersect1d(con_order, cont_many, return_indices=True)[1]
        )
    ...
    while len(tensor_list) > 1:
        ...
        tensor_list[-2] = np.outer(
            tensor_list[-2].reshape(np.prod(s1)), tensor_list[-1].reshape(np.prod(s2))
        ).reshape(np.append(s1, s2))
